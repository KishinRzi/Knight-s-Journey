...
unit_t *create_hero_combat(char *path, int hp)
{
    ...
    sfSprite_setOrigin(player->sprite,
        (sfVector2f){HERO_SIZE_W / 2, HERO_SIZE_H / 2});
...
unit_t *create_unit(char *path, int hp)
{
    ...
    sfSprite_setTextureRect(player->sprite, player->rect);
    sfSprite_setOrigin(player->sprite,
        (sfVector2f){UNIT_SIZE / 2, UNIT_SIZE / 2});
    player->pos = (sfVector2f){WIND_W / 2, WIND_H / 2};
    sfSprite_setPosition(player->sprite, player->pos);
...
static combat_gui_t *create_combat_gui(void)
{
    ...
    sfSprite_setOrigin(combat_gui->frame_sprite,
        (sfVector2f){FRAME_SIZE / 2, FRAME_SIZE / 2});
...
static combat_t *create_combat(void)
{
    ...
    sfSprite_setOrigin(combat->player->sprite,
        (sfVector2f){UNIT_SIZE / 2, UNIT_SIZE / 2});
...
rpg_t *init_rpg(void)
{
    ...
    sfSprite_setScale(rpg->player->sprite, (sfVector2f){1.5, 1.5});
